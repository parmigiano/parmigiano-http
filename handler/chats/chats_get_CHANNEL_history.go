...
func (h *Handler) ChatsGetChannelHistoryHandler(w http.ResponseWriter, r *http.Request) error {
	ctx := r.Context()
	...
	if chat.ChatType != "channel" {
		return httperr.BadRequest("этот чат не является каналом")
	}
	...
	if !ok {
		return httperr.Forbidden("вы не состоите в этом канале")
	}
