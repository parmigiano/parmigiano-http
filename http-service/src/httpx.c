...
static void rabbitmq_retry_pause(httpx_rabbitmq_worker_t *worker)
{
    for (int i = 0; i < 50; ++i)
    {
        ...
        {
            if (errno != EINTR || atomic_load(worker->stop))
                return;
        }
    }
}
...
static rmq_action_t rabbitmq_dispatch(const rmq_message_t *message, void *userdata)
{
    ...
    worker->last_action = worker->handler(message, worker->handler_context);
...
static void *rabbitmq_worker_main(void *arg)
{
    ...
    rmq_config_t config = {
        ...
        .rpc_timeout_ms = 5000,
        /* Moderation may run several bounded NSFW/ffmpeg operations before ACK. */
