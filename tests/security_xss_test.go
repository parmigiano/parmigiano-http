...
func TestSecurityMiddleware_XSSBlocked_MultiplePlaces(t *testing.T) {
	...
	payloads := []struct {
		...
		{"SimpleScript", "<script>alert('xss')</script>", true},
		{"ImgOnError", `<img src=x onerror=alert('xss')>`, true},
		{"InlineJS", `<div onclick="javascript:alert('xss')">click</div>`, true},
		{"HrefJS", `<a href="javascript:alert('xss')">link</a>`, true},
		{"EventHandler", `<button onmouseover="alert('xss')">hover</button>`, true},
